...
function validateResponse(testCase, response, tempRoot) {
  ...
  for (const marker of [TEST_EMAIL, INVALID_EMAIL, TEST_PASSWORD_MARKER, LONG_NAME]) {
...
function sanitizeFailureText(value, tempRoot) {
  ...
  for (const marker of [TEST_EMAIL, INVALID_EMAIL, 'rfq-recipient@example.invalid', TEST_PASSWORD_MARKER, LONG_NAME]) {
...
async function main() {
  ...
  try {
    ensure(TEST_CASES.length === 8, `Runtime matrix must contain exactly 8 cases; found ${TEST_CASES.length}.`);
    ensure(
    ...
    ensure(passedCases === TEST_CASES.length, 'Runtime test matrix did not complete all eight cases.');
    ensure(networkAudit.httpRequests === 8, `Runtime verification made ${networkAudit.httpRequests} HTTP request(s), expected exactly 8.`);
    await assertRateDirectory(withoutSmtpDirectory, 1);
    await assertRateDirectory(loopbackSmtpDirectory, 4);
