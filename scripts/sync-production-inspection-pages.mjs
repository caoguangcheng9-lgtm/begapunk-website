...
const data = {
  en: {
    ...
    evidenceTitle: 'Production inspection evidence', evidenceIntro: 'The two photographs document the inspection queue and one individual test cycle. Their limits are stated explicitly.',
    queueAlt: 'Assembled pneumatic rotary unions waiting in the production inspection queue', queueCaption: 'Assembled units awaiting inspection. The photograph does not establish that the pictured units have already passed.',
    passAlt: 'Leak-test station showing PASS during one individual passage test cycle', passCaption: 'PASS is shown for the pictured individual cycle only; this is not a batch test certificate.',
    boundary: 'The 100% inspection procedure is a confirmed current production process; it is not inferred from a single photograph. The image does not disclose the alarm threshold, leakage rate or calibration status.',
    ...
    cta: 'Request an Engineering Review →', back: '← Back to Manufacturing &amp; Quality', statement: 'Confirmed current production process',
  },
  de: {
    dir: 'de', home: 'Startseite', quality: 'Qualität', current: '100%-Dichtheitsprüfung', eyebrow: 'Produktionsprüfung', statement: 'Bestätigter Umfang',
    ...
    rows: [['Prüfzeitpunkt', 'Nach der Endmontage'], ['Prüfumfang', '100 % der Fertigteile'], ['Kanalabdeckung', 'Jeder Kanal wird einzeln geprüft'], ['Medium und Druck', 'Druckluft bei 1,0 MPa (etwa 10 bar)'], ['Zustand der übrigen Kanäle', 'Drucklos und offen'], ['Prüfzyklus', 'Etwa 1 Sekunde Druckaufbau plus 4 Sekunden Druckhaltezeit'], ['NG-Behandlung', 'Gelber Sperrbehälter; Ursache prüfen; instand setzen und vollständig nachprüfen oder verschrotten'], ['Freigabebedingung', 'Alle Kanäle müssen vor Verpackung und Einlagerung bestanden sein']],
    evidenceTitle: 'Fotodokumentation der Produktionsprüfung', evidenceIntro: 'Die beiden Fotos dokumentieren Prüfwarteschlange und einen einzelnen Prüfzyklus. Die Aussagegrenzen sind ausdrücklich angegeben.',
    queueAlt: 'Montierte pneumatische Drehdurchführungen in der betrieblichen Prüfwarteschlange', queueCaption: 'Montierte Einheiten vor der Prüfung. Das Foto belegt nicht, dass die abgebildeten Teile bereits bestanden haben.',
    passAlt: 'Dichtheitsprüfstation mit PASS-Anzeige bei einem einzelnen Kanalprüfzyklus', passCaption: 'Die angezeigte PASS-Meldung gilt nur für den abgebildeten einzelnen Prüfzyklus; das Foto ist kein Prüfzeugnis für ein Fertigungslos.',
    boundary: 'Die 100%-Prüfung ist als derzeit eingesetzter Produktionsablauf bestätigt und nicht aus einer einzelnen Fotografie abgeleitet. Alarmgrenzwert, Leckrate und Kalibrierstatus sind aus dem Bild nicht ableitbar.',
  ...
  ja: {
    dir: 'ja', home: 'ホーム', quality: '品質管理', current: '全数漏れ検査', eyebrow: '量産検査', statement: '確認できる範囲', statementSeparator: '：',
    ...
    evidenceTitle: '量産検査の写真資料', evidenceIntro: '2点の写真は、検査待ち工程と1回の個別検査サイクルを示します。写真から確認できる範囲を明記しています。',
    queueAlt: '量産検査待ちの組立済み空圧用ロータリージョイント', queueCaption: '検査前の組立済み製品です。写真だけでは、写っている製品が合格済みであることを示しません。',
    passAlt: '1流路の個別検査サイクルでPASSを表示する漏れ検査装置', passCaption: 'PASS表示は写真に写る1回の検査サイクルのみを示し、ロット全体の検査証明書ではありません。',
    boundary: '全数検査は、現在実施している量産工程として確認されたものであり、1枚の写真だけから推定したものではありません。写真から警報しきい値、漏れ量、校正状態を判断することはできません。',
  ...
  ru: {
    dir: 'ru', home: 'Главная', quality: 'Качество', current: '100%-ный контроль герметичности', eyebrow: 'Производственный контроль', statement: 'Границы подтверждения',
    ...
    processTitle: 'Производственный контроль в пять этапов', processIntro: 'Ниже показана полная последовательность — от приёмочного решения до действий с несоответствующим изделием.',
    ...
    evidenceTitle: 'Фотоматериалы производственного контроля', evidenceIntro: 'Две фотографии показывают очередь на контроль и один отдельный цикл проверки. Пределы интерпретации этих фотографий указаны явно.',
    queueAlt: 'Собранные пневматические вращающиеся соединения в очереди на производственный контроль', queueCaption: 'Собранные изделия до контроля. Фотография не подтверждает, что показанные изделия уже прошли проверку.',
    passAlt: 'Установка контроля герметичности показывает PASS для одного цикла проверки канала', passCaption: 'PASS относится только к показанному отдельному циклу и не является протоколом проверки партии.',
    boundary: 'Порядок 100%-ного контроля подтверждён как действующий производственный процесс и не выведен из одной фотографии. По изображению нельзя установить порог сигнализации, величину утечки или состояние калибровки.',
